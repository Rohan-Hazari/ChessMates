...
export default function ValidChessBoard({ fen }: { fen: string }) {
  ...
  const isValidMove = (
    ...
    if (endPiece && (endPiece === endPiece.toUpperCase()) === isWhitePiece)
      return false;
    ...
    switch (piece.toLowerCase()) {
      case "p":
        ...
        if (Math.abs(colDiff) === 1 && rowDiff === direction && endPiece)
          return true;
        return false;
      case "r":
      ...
      case "b":
        return (
          Math.abs(rowDiff) === Math.abs(colDiff) && !hasObstacles(start, end)
        );
      case "q":
        return (
          (rowDiff === 0 ||
  ...
  const handleClick = (row: number, col: number) => {
    if (selectedPiece) {
      ...
      if (
        piece &&
        ((piece === piece.toUpperCase() && currentPlayer === "white") ||
  ...
  return (
    <div className="flex flex-col items-start  bg-[#f0d9b5]">
      ...
      <Button
        onClick={resetGame}
        className="mt-6 bg-[#8b4513] text-white hover:bg-[#a0522d]"
      >
        Reset Game
      </Button>
    </div>
