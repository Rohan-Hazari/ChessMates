...
const CommentsSection = async ({ postId }: CommentsSectionProps) => {
  ...
  return <div className='flex flex-col gap-y-4 mt-4'>
    ...
    <div className='flex flex-col gap-y-6 mt-4 justify-center '>
      ...
      {comments.length > 0 ? (comments.filter((comment) => !comment.replytoId).map((topComment) => {
        ...
        return <div key={topComment.id} className='flex flex-col'>
          <CommentsContainer topComment={topComment} topCommentVote={topCommentVote}
            topCommentVotesAmt={topCommentVotesAmt} postId={postId} />
        </div>
      })) : (<p className='text-center text-zinc-400'>No comments yet. Be the first to comment</p>)}
    </div>
  </div>
}
